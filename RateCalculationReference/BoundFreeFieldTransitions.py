...
class BoundFreeFieldTransitions:
    ...
    def n_eff_numpy(Z, E_Ip):
        ...
        :returns: effective principal quantum number
        """
        return np.float32(Z / np.sqrt(2. * E_Ip))
    ...
    def ADKRate_numpy(Z, E_Ip, F):
        ...
        dBase = np.float64(4. * np.exp(1.) * Z**3. / (F * nEff**4.))
        ...
        gamowFactor = np.exp(-(2. * Z**3.) / (3. * nEff**3. * F))
        print("\t\t gamowFactor: {:.4e}".format(gamowFactor))

        rate = F * np.float32(D**2. * gamowFactor) * np.sqrt((3. * nEff**3. * F) / (np.pi * Z**3.)) / (8. * np.pi * Z)
        return rate
    ...
    def ADKRate_mpmath(Z, E_Ip, F):
        ...
        rate = (F * D**mp.mpf(2)) / (mp.mpf(8) * mp.pi * Z) * mp.exp(-(mp.mpf(2) * Z**mp.mpf(3)) / (mp.mpf(3) * nEff**mp.mpf(3) * F)) * mp.sqrt((mp.mpf(3) * nEff**mp.mpf(3) * F) / (mp.pi * Z**mp.mpf(3)))
