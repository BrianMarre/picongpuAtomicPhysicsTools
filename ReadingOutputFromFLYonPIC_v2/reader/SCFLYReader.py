...
class SCFLYReader(StateDistributionReader):
    ...
    def readSCFLYNames(self) -> tuple[dict[str, int], dict[int, str]]:
        """
        ...
        for levelVector in conversionTable[ids]:
            configNumbers.append(conv.getConfigNumber(levelVector, self.atomicNumber))
        ...
        SCFLYStateName_to_atomicConfigNumber[str(self.atomicNumber)+"+"] = 0
    ...
    def check(self) -> None:
        ...
        if self.numLevels <= 0:
            raise ValueError("numLevels must be > 0")

    def getSCFLYStateNameRegex(self, SCFLY_to_FLYonPIC : dict[str, int]) -> str, int:
    ...
    def getAllUniqueBlockSizes(self, stateName_regex : str) -> tuple[
    ...
    def readInDataForEachUniqueBlockSize(self, uniqueBlockSizes : npt.NDArray[np.uint32], stateName_regex : str) -> tuple[
        ...
        for blockSize in uniqueBlockSizes:
            regex_StateEntries, regex_TimeEntry, dtype_StateEntries, dtype_TimeEntry, blockIds = (
                self.getSCFLYOutputLineRegex(stateName_regex, blocksize))
    ...
    def read(self) -> tuple[
            ...
            tuple[npt.NDArray[np.float64], npt.NDArray[np.uint64]],
            dict[str, int],
            tuple[]]:
        """
        ...
        uniqueBlockSizes, totalNumberSteps, blockShapeDescriptions = self.getAllUniqueBlockSizes(stateName_regex)
